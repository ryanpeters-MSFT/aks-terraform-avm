...
- Three-node `testpool` user pool using the AKS `VirtualMachines` agent pool type with two manual profiles: two `Standard_D4ds_v5` nodes and one `Standard_D8ds_v5` node
...
- Sufficient Central US quota for up to eleven `Standard_D4ds_v5` VMs and one `Standard_D8ds_v5` VM during normal autoscaling, plus temporary upgrade surge capacity
